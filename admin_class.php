...
Class Action {
	...
	function save_visa_a(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO page_one set $data");
			$id=$this->db->insert_id;
		}else{
			$save = $this->db->query("UPDATE page_one set $data where id = $id");
		}  
	...
	function delete_visa_a(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM page_one where id = ".$id);
		if($delete){
			return 1;
		}
	}
